...
function grabProject(context, next) {
  if (context.module.clone_repo) {
    grabGitRepository(context, next);
  } else {
    grabRawSource(context, next);
  }
}

function grabGitRepository(context, next) {
  let bailed = false;
  const options = createOptions(context.path, context);
  const proc = spawn('git', ['clone', '-b', context.module.branch, context.module.repo, context.module.name],
        options);
  ...
  proc.on('close', function(code) {
    ...
    context.emit('data', 'info', context.module.name + ' git:',
        'git clone -b ' + context.module.branch + ' ' + context.module.repo +
            ' into ' + context.module.name);
    return next(null, context);
  });
}

function grabRawSource(context, next) {
  if (context.meta)
