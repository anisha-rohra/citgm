...
function test(context, next) {
  ...
  if (context.module.skip_tests) {
    context.emit('data', 'info', context.module.name + ' file:',
        'removing skipped tests');
    if (context.module.skip_tests instanceof Array) {
      // Remove skip test files
      context.module.skip_tests.forEach(function (file) {
        fs.unlinkSync(path.join(wd, file));
      });
    }
  }
  ...
  readPackage(path.join(wd, 'package.json'), false, function(err, data) {
    ...
    if (data.dependencies['node-addon-api'] && process.versions.modules >= 52) {
      // A dependency on the N-API package was detected, and the
      // Node version supports N-API.
      // So enable N-API modules when running tests.
      options.env.NODE_OPTIONS = '--napi-modules';
    }
    const proc = spawn('npm', ['test'], options);
